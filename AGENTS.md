...
- 各物件フォルダ内の標準構成は `PROPERTY_FOLDER_STRUCTURE` で定義する。子階層を追加する場合は対象要素に `children: [{ name: '子フォルダ名' }]` を追加し、`ensureFolderStructure_()` の再帰処理に作成させる。
...
- NotebookLMとの連携（`80_AI参照用` フォルダ）は手動運用。外部から自動でノートブックを作成・更新する公式APIが現時点の個人/Workspace通常プランには無いため。
